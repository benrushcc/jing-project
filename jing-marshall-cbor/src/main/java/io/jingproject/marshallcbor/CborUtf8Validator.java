...
public final class CborUtf8Validator {
    ...
    // current UTF-8 validation implementation provides a vectorised version
    // using SIMD APIs and a scalar version that processes bytes one by one.
    // on x86, the vector path usually outperforms the scalar path by a large margin.
    // on ARM with NEON only, the lack of dedicated mask registers forces the
    // vector implementation to simulate masking via extra boolean arrays,
    // which introduces a notable performance regression. Therefore, when the
    // platform is not amd64, we avoid using mask and employ rearrange operations
    // as an alternative to work around this limitation.
    private static final boolean NON_MASK;
    ...
    static {
        ...
        int vecSize = Integer.parseInt(System.getProperty("jing.marshallcbor.utf8validator.vecsize", "-1"));
        if (vecSize < 0) {
        ...
        NON_MASK = arch == null || arch.contains("aarch") || arch.contains("arm");
    }

    private CborUtf8Validator() {
    ...
    public static boolean validateHeap(byte[] bytes, int from, int to) {
        ...
        for (; from <= to - BYTE_SPECIES.length(); from += BYTE_SPECIES.length()) {
            ...
            if (chunk.and(ALL_ASCII_MASK).eq((byte) 0).allTrue()) {
                ...
                ByteVector prev1 = ((ByteVector) chunkAsInts.lanewise(VectorOperators.LSHL, 8)
                        .or(shifted.lanewise(VectorOperators.LSHR, 24)).reinterpretShape(BYTE_SPECIES, 0));
                ByteVector highNibbles2 = ((ByteVector) chunkAsInts.lanewise(VectorOperators.LSHR, 4)
                        .reinterpretShape(BYTE_SPECIES, 0)).and(LOW_NIBBLE_MASK);
                ByteVector highNibbles1 = ((ByteVector) ((IntVector) prev1.reinterpretShape(INT_SPECIES, 0))
                        .lanewise(VectorOperators.LSHR, 4).reinterpretShape(BYTE_SPECIES, 0)).and(LOW_NIBBLE_MASK);
                ByteVector lowNibbles1 = prev1.and(LOW_NIBBLE_MASK);
                ByteVector sc = highNibbles1.selectFrom(BYTE1_HIGH_TABLE)
                        .and(lowNibbles1.selectFrom(BYTE1_LOW_TABLE))
                        .and(highNibbles2.selectFrom(BYTE2_HIGH_TABLE));
                ByteVector prev2 = ((ByteVector) chunkAsInts.lanewise(VectorOperators.LSHL, 16)
                        .or(shifted.lanewise(VectorOperators.LSHR, 16)).reinterpretShape(BYTE_SPECIES, 0));
                ByteVector prev3 = ((ByteVector) chunkAsInts.lanewise(VectorOperators.LSHL, 24)
                        .or(shifted.lanewise(VectorOperators.LSHR, 8)).reinterpretShape(BYTE_SPECIES, 0));
    ...
    public static boolean validateSegment(MemorySegment segment, long from, long to) {
        ...
        int previousFourBytes = 0;
        for( ; from <= to - BYTE_SPECIES.length(); from += BYTE_SPECIES.length()) {
            ByteVector chunk = ByteVector.fromMemorySegment(BYTE_SPECIES, segment, from,
                    ByteOrder.nativeOrder()); // byteOrder will be ignored
            ...
            if (chunk.and(ALL_ASCII_MASK).eq((byte) 0).allTrue()) {
                ...
                previousIncomplete = chunk.compare(VectorOperators.UGE, INCOMPLETE).toLong();
                IntVector shifted = chunkAsInts.rearrange(FOUR_BYTES_FORWARD_SHIFT).withLane(0, previousFourBytes);
                ByteVector prev1 = chunkAsInts.lanewise(VectorOperators.LSHL, 8)
                        .or(shifted.lanewise(VectorOperators.LSHR, 24)).reinterpretAsBytes();
                ByteVector highNibbles2 = chunkAsInts.lanewise(VectorOperators.LSHR, 4)
                        .reinterpretAsBytes().and(LOW_NIBBLE_MASK);
                ByteVector highNibbles1 = prev1.reinterpretAsInts().lanewise(VectorOperators.LSHR, 4)
                        .reinterpretAsBytes().and(LOW_NIBBLE_MASK);
                ByteVector lowNibbles1 = prev1.and(LOW_NIBBLE_MASK);
                ByteVector sc = highNibbles1.selectFrom(BYTE1_HIGH_TABLE)
                        .and(lowNibbles1.selectFrom(BYTE1_LOW_TABLE))
                        .and(highNibbles2.selectFrom(BYTE2_HIGH_TABLE));
                ByteVector prev2 = chunkAsInts.lanewise(VectorOperators.LSHL, 16)
                        .or(shifted.lanewise(VectorOperators.LSHR, 16)).reinterpretAsBytes();
                ByteVector prev3 = chunkAsInts.lanewise(VectorOperators.LSHL, 24)
                        .or(shifted.lanewise(VectorOperators.LSHR, 8)).reinterpretAsBytes();
                VectorMask<Byte> must23 = prev2.compare(VectorOperators.UGT, MAX_2_BYTE_LEAD)
                        .or(prev3.compare(VectorOperators.UGT, MAX_3_BYTE_LEAD));
                errors |= sc.add((byte) 0x80, must23).compare(VectorOperators.NE, (byte) 0).toLong();
            }
        ...
        if (from < to) {
            ByteVector chunk;
            if(NON_MASK) {
                chunk = ByteVector.fromMemorySegment(BYTE_SPECIES, segment,
                        to - BYTE_SPECIES.length(), ByteOrder.nativeOrder())
                        .slice(BYTE_SPECIES.length() - Math.toIntExact(to - from));
            } else {
                chunk = ByteVector.fromMemorySegment(BYTE_SPECIES, segment, from,
                        ByteOrder.nativeOrder(),
                        BYTE_SPECIES.indexInRange(from, to)); // byteOrder will be ignored
            }
            IntVector chunkAsInts = chunk.reinterpretAsInts();
            if (!chunk.and(ALL_ASCII_MASK).eq((byte) 0).allTrue()) {
                previousIncomplete = chunk.compare(VectorOperators.UGE, INCOMPLETE).toLong();
    ...
    public static boolean scalarValidateHeap(byte[] bytes, int from, int to) {
        if (from < 0 || from > to || to > bytes.length) {
            throw new IndexOutOfBoundsException("range [" + from + ", " + to
    ...
    public static boolean scalarValidateSegment(MemorySegment segment, long from, long to) {
        if (from < 0 || from > to || to > segment.byteSize()) {
            throw new IndexOutOfBoundsException("range [" + from + ", " + to
                    + ") out of bounds for length : " + segment.byteSize());
        }
        int b1, b2;
        for (; ; ) {
            do {
