...
int jing_win_ansi_support(void) {
  ...
  if((mode & ENABLE_PROCESSED_OUTPUT) && (mode & ENABLE_VIRTUAL_TERMINAL_PROCESSING)) {
    return 0;
  }
...
void jing_create_file(LPCWSTR filename, jing_result* r) {
  HANDLE v = CreateFileW(filename, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  if(JING_UNLIKELY(v == INVALID_HANDLE_VALUE)) {
    int err = GetLastError();
    jing_err_result(r, err);
  } else {
...
void jing_write_file(HANDLE h, char* buffer, int len, jing_result* r) {
  ...
  while (total < length)
  {
    if(JING_LIKELY(WriteFile(h, buffer + total, length - total, &written, NULL))) {
      total += written;
    } else {
      int err = GetLastError();
      jing_err_result(r, err);
...
int jing_wepoll_ctl_add(void) { return EPOLL_CTL_ADD; }

int jing_wepoll_ctl_mod(void) { return EPOLL_CTL_MOD; }

int jing_wepoll_ctl_del(void) { return EPOLL_CTL_DEL; }
