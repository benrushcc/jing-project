...
static inline void jing_err_result_with_flag(jing_result *r, int err, int flag) {
  memset(r, 0, sizeof(jing_result));
  r->len = 0;
  r->data.err_val.err_code = err;
...
JING_EXPORT_SYMBOL void jing_write_file(HANDLE h, char* buffer, int len, jing_result* r);
...
JING_EXPORT_SYMBOL void jing_wepoll_ctl(HANDLE epfd, SOCKET socket, int op, uint32_t events, int data, jing_result *r);

JING_EXPORT_SYMBOL void jing_wepoll_wait(HANDLE epfd, struct epoll_event *events, int maxevents, int timeout, jing_result *r);
...
JING_EXPORT_SYMBOL void jing_write_fd(int fd, char* buf, size_t len, jing_result* r);
...
JING_EXPORT_SYMBOL void jing_socket(int af, int type, int protocol, jing_result *r);
...
JING_EXPORT_SYMBOL int jing_ssl_ctx_set_min_proto_version(SSL_CTX *ctx, uint16_t version);

JING_EXPORT_SYMBOL int jing_ssl_ctx_set_max_proto_version(SSL_CTX *ctx, uint16_t version);
